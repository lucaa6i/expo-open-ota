...
2. **Caching the computed manifest**
   - Manifest generation can be an expensive operation, especially when querying the database.
...
- Reduce the load on database queries and CPU-intensive computations.
